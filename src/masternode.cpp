...
{
    if (nHeight >  2000000) return    200000 * COIN;
    if (nHeight >  1525000) return    150000 * COIN;
    if (nHeight >  1000000) return    100000 * COIN;
    if (nHeight >   525000) return     50000 * COIN;
    if (nHeight >   200000) return     40000 * COIN;
    if (nHeight >    50000) return     30000 * COIN;
    if (nHeight >        0) return     25000 * COIN;
...
{
    if (nHeight >   2525000) return        1 * COIN;
    if (nHeight >   2000000) return       20 * COIN;
    if (nHeight >   1525000) return       25 * COIN;
    if (nHeight >   1000000) return       27 * COIN;
    if (nHeight >    525000) return       30 * COIN;
    if (nHeight >    200000) return       27 * COIN;
    if (nHeight >     50000) return       23 * COIN;
    if (nHeight >       200) return       18 * COIN;
    if (nHeight >         0) return    37500 * COIN;
...
{
    if(nHeight > 569598) return GetBlockValue(nHeight) * 95 / 100;
    if(nHeight > 568598) return 0; // give everybody a chance to get their MN online 
    if(nHeight > 200) return GetBlockValue(nHeight) * 95 / 100;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 STUDS tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 STUDS tx -> 1 confirmation
