...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSTUDSStake(spend));
    } else {
        ...
        CSTUDSStake* STUDSInput = new CSTUDSStake();
        STUDSInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(STUDSInput);
    }
