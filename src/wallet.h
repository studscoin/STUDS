...
enum AvailableCoinsType {
    ...
    EXCEPT_DENOMINATED_OR_MASTERNODE_COLLATERAL = 4, // ONLY_NONDENOMINATED and not 15000 STUDS at the same time
    ONLY_MASTERNODE_COLLATERAL = 5,       // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zstuds send
enum ZerocoinSpendStatus {
    ZSTUDS_SPEND_OKAY = 0,                            // No error
    ZSTUDS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSTUDS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSTUDS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSTUDS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSTUDS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSTUDS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSTUDS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSTUDS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSTUDS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSTUDS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSTUDS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSTUDS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSTUDS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSTUDS_SPENT_USED_ZSTUDS = 14,                      // Coin has already been spend
    ZSTUDS_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSTUDS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZSTUDSBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZSTUDSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzstudsAuto) const;
    ...
    */
    ...
    CzstudsWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzstudsTracker> zstudsTracker;
    ...
    void setZWallet(CzstudsWallet* zwallet)
    {
        zwalletMain = zwallet;
        zstudsTracker = std::unique_ptr<CzstudsTracker>(new CzstudsTracker(strWalletFile));
    }

    CzstudsWallet* getZWallet() { return zwalletMain; }
    ...
    /** zstuds reset */
    boost::signals2::signal<void()> NotifyzstudsReset;
