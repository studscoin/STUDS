...
public:
    ...
    bool WriteZSTUDSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSTUDSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSTUDSSeed_deprecated(uint256& seed);
    bool EraseZSTUDSSeed();
    bool EraseZSTUDSSeed_deprecated();

    bool WriteZSTUDSCount(const uint32_t& nCount);
    bool ReadZSTUDSCount(uint32_t& nCount);
