...
CzstudsTracker::CzstudsTracker(std::string strWalletFile)
{
...
CzstudsTracker::~CzstudsTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzstudsTracker::Init()
{
...
bool CzstudsTracker::Archive(CMintMeta& meta)
{
...
bool CzstudsTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzstudsTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzstudsTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzstudsTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzstudsTracker::GetSerialHashes()
{
...
CAmount CzstudsTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzstudsTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzstudsTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzstudsTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzstudsTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzstudsTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzstudsTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzstudsTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzstudsTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzstudsTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzstudsTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzstudsTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzstudsTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzstudsTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzstudsTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzstudsTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzstudsTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzstuds from DB\n", __func__, listDeterministicDB.size());
    }
