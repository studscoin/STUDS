...
const char * const PIVX_CONF_FILENAME = "studscoin.conf";
const char * const PIVX_PID_FILENAME = "studs.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Studscoin only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Studscoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Studscoin
// Mac: ~/Library/Application Support/Studscoin
// Unix: ~/.studscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Studscoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\studsParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\studsParams
    // Mac: ~/Library/Application Support/studsParams
    // Unix: ~/.studs-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "StudscoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "StudscoinParams";
#else
    // Unix
    return pathRet / ".studs-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty studscoin.conf if it does not exist
        ...
        if (configFile != NULL) {
	        ...
	        strHeader += "\nrpcport=11221\naddnode=136.243.77.62:11220\naddnode=[2a01:4f8:212:2b15::157]:11220\naddnode=[2a01:4f8:212:2b15::158]:11220\n";
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override studscoin.conf
