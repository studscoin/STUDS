...
CzstudsWallet::CzstudsWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zstuds seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSTUDSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSTUDSSeed_deprecated()) {
                    LogPrintf("%s: Updated zstuds seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zstuds seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zstuds wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzstudsWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSTUDSCount(nCountLastUsed);
    else if (!walletdb.ReadZSTUDSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzstudsWallet::Lock()
{
    seedMaster = 0;
}

void CzstudsWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzstudsWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSTUDS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzstudsWallet::LoadMintPoolFromDB()
{
...
void CzstudsWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzstudsWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzstudsWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zstudsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzstudsWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSTUDS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zstudsTracker which also adds to database
    pwalletMain->zstudsTracker->Add(dMint, true);
...
void CzstudsWallet::SeedToZSTUDS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzstudsWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzstudsWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZSTUDSCount(nCountLastUsed);
...
void CzstudsWallet::GenerateDeterministicZSTUDS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzstudsWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzstudsWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
