...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::STUDS);
    ...
    if (!settings.contains("nAnonymizeStudscoinAmount"))
        settings.setValue("nAnonymizeStudscoinAmount", 1000);

    nAnonymizeStudscoinAmount = settings.value("nAnonymizeStudscoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeStudscoinAmount"))
        SoftSetArg("-anonymizestudscoinamount", settings.value("nAnonymizeStudscoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in studscoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeStudscoinAmount:
            return QVariant(nAnonymizeStudscoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeStudscoinAmount:
            nAnonymizeStudscoinAmount = value.toInt();
            settings.setValue("nAnonymizeStudscoinAmount", nAnonymizeStudscoinAmount);
            emit anonymizeStudscoinAmountChanged(nAnonymizeStudscoinAmount);
            break;
        case CoinControlFeatures:
