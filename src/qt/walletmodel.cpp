...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered studscoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 STUDS.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal studscoin:URI (studscoin:XyZ...?message=example)
            {
...
static void NotifyzstudsReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzstudsReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzstudsReset.connect(boost::bind(NotifyzstudsReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzstudsReset.disconnect(boost::bind(NotifyzstudsReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zstudsTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
