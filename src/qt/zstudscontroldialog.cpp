...
#include "ui_zstudscontroldialog.h"
...
std::set<std::string> ZSTUDSControlDialog::setSelectedMints;
std::set<CMintMeta> ZSTUDSControlDialog::setMints;

ZSTUDSControlDialog::ZSTUDSControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSTUDSControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSTUDSControlDialog::~ZSTUDSControlDialog()
{
    delete ui;
}

void ZSTUDSControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSTUDSControlDialog::updateList()
{
...
void ZSTUDSControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSTUDSControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSTUDS_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSTUDSControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSTUDSControlDialog::GetSelectedMints()
{
...
void ZSTUDSControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
