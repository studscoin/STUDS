...
#ifndef Studscoin_ZSTUDSWALLET_H
#define Studscoin_ZSTUDSWALLET_H
...
public:
    CzstudsWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSTUDS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSTUDS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Studscoin_ZSTUDSWALLET_H
