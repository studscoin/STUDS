...
#if defined(NDEBUG)
#error "Studscoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uSTUDS) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : zstuds spend with serial %s is already in block %d\n", __func__,
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	//Check to see if the zstuds is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zstuds spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zstuds without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : zstuds spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zstuds spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zstuds");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that zstuds mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 525000 && nHeight > 200000) {
		nSubsidy = 27 * COIN;
	}
	else if (nHeight <= 1000000 && nHeight > 525000) {
		nSubsidy = 30 * COIN;
	}
	else if (nHeight <= 1525000 && nHeight > 1000000) {
		nSubsidy = 27 * COIN;
	}
	else if (nHeight <= 2000000 && nHeight > 1525000) {
		nSubsidy = 25 * COIN;
	}
	else if (nHeight <= 2525000 && nHeight > 2000000) {
		nSubsidy = 20 * COIN;
	}
	else {
		nSubsidy = 1* COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZSTUDSStake)
{
    int64_t ret = 0;
...
int64_t GetMasternodePayment (int nHeight, unsigned int mnLevel, int64_t blockValue, int nMasternodeCount, bool isZSTUDSStake) {
  return GetMasternodePayment (nHeight, blockValue, nMasternodeCount, isZSTUDSStake) * Params ().getMasternodePhaseWeight (mnLevel, nHeight) / Params ().getMasternodePhaseWeight (0, nHeight);
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from Studscoin
...
{
	RenameThread("studscoin-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZSTUDSMinted()
{
...
void RecalculateZSTUDSSpent()
{
	...
	while (true) {
		...
		//Remove spends from zstuds supply
...
bool RecalculateSTUDSSupply(int nHeightStart)
{
...
{
	// Studscoin: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZSTUDSSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		RecalculateZSTUDSMinted();
		RecalculateZSTUDSSpent();
		RecalculateSTUDSSupply(Params().Zerocoin_StartHeight());
	}

	//Track zstuds money supply in the block index
	if (!UpdateZSTUDSSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zstuds supply for block=%s height=%d", __func__,
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSTUDSSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zstuds spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zstuds serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zstuds stake block is less than allowed start height", __func__);

	if (CZSTUDSStake* zstuds = dynamic_cast<CZSTUDSStake*>(stake)) {
		CBlockIndex* pindexFrom = zstuds->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zstuds stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zstuds stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zstuds->GetValue()));
		if (nChecksum200 != zstuds->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zstuds->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZSTUDS() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zstuds fails context checks", __func__));
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> STUDSInputs;
		std::vector<CTxIn> zstudsInputs;

		for (CTxIn stakeIn : stakeTxIn.vin) {
			if (stakeIn.scriptSig.IsZerocoinSpend()) {
				zstudsInputs.push_back(stakeIn);
			}
			else {
				STUDSInputs.push_back(stakeIn);
			}
		}
		const bool hasSTUDSInputs = !STUDSInputs.empty();
		const bool hasZSTUDSInputs = !zstudsInputs.empty();
		...
		for (CTransaction tx : block.vtx) {
			for (CTxIn in : tx.vin) {
				...
				if (hasSTUDSInputs)
					...
					for (CTxIn STUDSIn : STUDSInputs) {
						if (STUDSIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			// Now that this loop if completed. Check if we have zstuds inputs.
			if (hasZSTUDSInputs) {
				for (CTxIn zSTUDSInput : zstudsInputs) {
					CoinSpend spend = TxInToZerocoinSpend(zSTUDSInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zstuds");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zstuds mints and %d zstuds spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// Studscoin: We use certain sporks during IBD, so check to see if they are
...
int ActiveProtocol()
{
	// SPORK_14 was used for 90100 (v2.0.0+)
	     //if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
	        //return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

	// SPORK_15 is used for 90101 (v2.1.0+)
