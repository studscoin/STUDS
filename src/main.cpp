...
/** Fees smaller than this (in uSTUDS) are considered zero fee (for relaying, mining and transaction creation)
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zSTUDS mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zSTUDS mint output in tx %s",
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    //Track zSTUDS money supply
    if (!UpdateZPIVSupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zSTUDS supply", __func__);
        return DISCONNECT_FAILED;
    }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // Update zSTUDS money supply map
    ...
    // A one-time event where the zSTUDS supply was off (due to serial duplication off-chain on main net)
...
int ActiveProtocol()
{
	// SPORK_14 was used for 90102 (v2.2.0+)
