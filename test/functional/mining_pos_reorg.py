...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected STUDS and zSTUDS supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zSTUDSsupply']:
                assert_equal(node['zSTUDSsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check STUDS and zSTUDS supply at the beginning
        # ------------------------------------------
        # zSTUDS supply: 2 coins for each denomination
        ...
        # STUDS supply: block rewards minus burned fees for minting
        ...
        # Verify that STUDS and zSTUDS supplies were properly updated after the spends and reorgs
        self.log.info("Check STUDS and zSTUDS supply...")
